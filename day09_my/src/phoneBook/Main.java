...
public class Main {
	...
	// 데이터를 저장할 배열은 어느 함수에서나 접근할 수 있도록 static으로 작성
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 3:			// 수정
				...
				System.out.print("전화번호 입력 >> ");
				pnum = sc.nextLine();
//				tmp.idx = sequence++;	// 고유번호까지 변경하려면
				tmp.idx = idx;			// idx를 유지하려면 (신규생성이 아니므로 이쪽으로)
				tmp.name = name;
				tmp.pnum = pnum;
			...
			default:
				System.out.println("메뉴를 확인해 주시고 다시 입력해주세요");
				break;
			}
			System.out.println();
	...
	// 특정 데이터를 추가할 때 사용하는 함수
	static int insert(Contact ob) {	// 빈칸을 찾아서, ob를 저장
		int row =0;
		for(int i=0; i<arr.length;i++) {
			if(arr[i] == null) {
				arr[i] = ob;
