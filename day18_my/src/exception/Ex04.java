...
class Timer {
	...
	public void start() throws Exception {	// 발생하는 예외를 caller에게 전가시키겠다.
		for(int i=(int) minute *60; i!= -1; i--) {
			System.out.printf("[%02d : %02d]\n",i/60, i%60);
			Thread.sleep(1000);  // 쓰레드를 제어하는 과정에서 예외가 발생할 수 있다.
		}
	}
}
...
public class Ex04 {
	public static void main(String[] args) {
		...
		try {
			...
			// print : 출력. stack : 쌓이는 구조(함수 스택), trace : 경로, 추적
		} finally {
			sc.close();
		}
		
		// 프로그램의 규모가 커지고, 호출하는 함수의 개수와 스택이 커지면
		// 일일이 처리하는 것보다, throws를 통해서 예외를 한곳에 집중시켜서 처리하는 것이 효율적
