...
public class Ex01 {
	public static void main(String[] args) throws IOException {
		...
		// IP주소 : 건물 이름
		// 포트 번호 : 객실 번호
		// 주소와 포트를 지정하면 대상 컴퓨터에서 실행되는 프로세서와 연결하여 데이터를 주고 받는다.
		...
		bw.newLine();					// 한줄 바꾼다.
		bw.flush();						// flush에서 버퍼 내용을 밀어서 보낸다.
		
		bw.close();
		sc.close();
		so.close();						// 사용이 끝난 스트림 및 소켓은 반드시 닫는다.
		
		// 이 코드만 사용할 시 서버에 연결이 안되어 있기 때문에 에러가 난다.
	}
}
